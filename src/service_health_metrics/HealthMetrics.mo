...
actor HealthMetrics = {
	...
	var logs_ordered = HashMap.HashMap<Text, Buff.Buffer<Log>>(
		0,
		Text.equal,
		Text.hash
	);
	...
	public shared (msg) func log_event(log_payload : Payload) : async () {
		...
		switch (logs_ordered.get(log_payload.child_canister_id)) {
			case null {
				let logs_buffer = Buffer<Log>(0);
				logs_buffer.add(log);
				logs_ordered.put(log_payload.child_canister_id, logs_buffer);
			};
			case (?logs_buffer) {
				logs_buffer.add(log);
				logs_ordered.put(log_payload.child_canister_id, logs_buffer);
